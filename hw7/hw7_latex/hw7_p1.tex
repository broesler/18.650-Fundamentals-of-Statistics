...
\begin{proof}
  ...
  \begin{thm}[Weak Law of Large Numbers] \label{eq:LLN}
...
\begin{thm}[Central Limit Theorem]
    ...
    and $\overline{X}_n = \avg{i}{n} X_i$.
    Then,
    \[ \sqrt{n}(\overline{X}_n - \E{X}) \Dlim \N{0}{\Sigma}. \]
...
\begin{prop}
  ...
  \begin{equation}
    T_n \coloneqq \sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{\hat{V}}} \sim t_{n-1}
  \end{equation}
...
\begin{proof}
  ...
  \label{eq:Tn_norm}
\end{alignat*}
The asymptotic variance $V$, however, is unknown, so we substitute the estimator
$\hat{V}$ to get the test statistic
\[ T_n \coloneqq  \sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{\hat{V}}}. \]
If we rewrite the test statistic in terms of the unknown variance,
\begin{equation} \label{eq:t1}
  \sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{\hat{V}}} =
  \ddfrac{\sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{V}}}
         {\sqrt{\frac{\hat{V}}{V}}} 
\end{equation}
Equation~\eqref{eq:Tn_norm} shows that the numerator is a standard normal random
variable. Cochran's theorem gives the distribution of the denominator
\begin{thm}[Cochran's Theorem]
  If $X_1, \dots, X_n$ are \iid\ random variables drawn from the distribution
  $\N{\mu}{\sigma^2}$, and $S_n \coloneqq \avg{i}{n} (X_i - \overline{X}_n)^2$,
  then
  \[ \overline{X}_n \indep S_n \]
  and
  \[ \frac{n S_n}{\sigma^2} \sim \chi_{n-1}^2. \]
\end{thm}
% where $S_n$ is the sample variance of $n$ \iid\ normal random variables 
  % \sim \ddfrac{\N{0}{1}}{\sqrt{\frac{\chi_{n-1}^2}{\nu}}}
  % \implies T_n &\Dlim t_{n-1}
\end{proof}

We define the rejection region of our test
\begin{equation}
