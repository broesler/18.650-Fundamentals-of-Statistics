...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{Rank}{$A, k$}
      ...
      \State\Return $\#\{i=1,\dots,\text{length}(A): k < A_i\}$
    \EndFunction
...
\begin{proof}
  ...
  \begin{alignat*}{3}
    \Var{\phat} &= \Var{\avg{i}{n} X_i} \\
                &= \frac{1}{n^2}\Var{\sumi{i}{n} X_i} \by{variance rule} \\
                &= \frac{1}{n^2}\sumi{i}{n}\Var{X_i} \by{\iid} \\
                &= \frac{1}{n^2}n\Var{X_1} \by{\iid} \\
   \therefore \Var{\phat} &= \frac{1}{n} p(1-p) \by{variance of $\Ber(p)$} \qedhere
  \end{alignat*}
\end{proof}
...
\clearpage
The covariance of $\phat$ and $\qhat$ is given by 
\begin{equation}
  \Cov{\phat, \qhat} = \E{\phat\qhat} - \E{\phat}\E{\qhat}
\end{equation}

\begin{prop}
  The expectation of the estimator $\phat$ is given by $\E{\phat} = p$.
\end{prop}

\begin{proof}
  \begin{align}
    \E{\phat} &= \E{\avg{i}{n} X_i} \\
              &= \frac{1}{n} \sum{i}{n} \E{X_i} \by{linearity of expectation} \\
              &= \frac{1}{n} n \E{X_1} \by{\iid} \\
    \implies \E{\phat} &= p \by{expectation of $\Ber(p)$} \qedhere
  \end{align}
\end{proof}
Incidentally, this proposition also shows that the estimator is \emph{unbiased},
since $\E{\phat - p} = 0$.

The covariance is then
\begin{equation}
  \Cov{\phat, \qhat} = \E{\phat\qhat} - pq.
\end{equation}
The expectation of the product $\phat\qhat$ is unknown since it is unknown if
$X \indep Y$.

\begin{prop}
  The covariance of $\phat$ and $\rhat$ is given by $\Cov{\phat, \rhat} = r(1
  - p)$.
\end{prop}
...
\begin{proof}
  \begin{alignat*}{3}
    ...
    &= \frac{1}{n^2} \Cov{\sumi{i}{n} X_i, \sumi{i}{n} R_i} \by{covariance rules} \\
    &= \frac{1}{n^2} \sumi{i}{n} \sumi{j}{n} \Cov{X_i, R_j} \by{bilinearity of covariance} \\
    &= \frac{1}{n^2} n^2 \Cov{X_1, R_1} \by{\iid} \\
    &= \Cov{X_1, R_1}  \\
    &= \E{X R} - \E{X}\E{R} \by{definition} \\
    &= \E{X R} - pr \by{given} \\
