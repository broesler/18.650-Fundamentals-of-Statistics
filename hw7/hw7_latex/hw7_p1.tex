...
\begin{proof}
  ...
  \[ \hat{V}(\phat, \qhat) \Plim V(p, q). \quad\text{(CMT)} \qedhere \]
\end{proof}

\subsubsection{A Hypothesis Test}
Given $\alpha \in (0, 1)$, we propose the test statistic
\begin{equation}
  T_n = \sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{V}}
\end{equation}
where $V$ is given by Proposition~\ref{prop:V_H0}. Under $H_0$,
Proposition~\ref{prop:delta} gives the distribution of the test statistic.
\begin{alignat*}{3}
  \sqrt{n}\mleft((\rhat - \phat\qhat) - (r - pq)\mright) &\Dlim \N{0}{V} \\
  \sqrt{n}(\rhat - \phat\qhat) &\Dlim \N{0}{V} \by{$r = pq$ under $H_0$} \\
  \sqrt{n}\frac{(\rhat - \phat\qhat)}{\sqrt{V}} &\Dlim \N{0}{1} \\
  \implies T_n &\Dlim \N{0}{1} 
\end{alignat*}
