...
statistic distribution has a much longer right tail than the standard normal.
...
is shown in Figure~\ref{fig:Sn} in comparison to a standard normal. The test
statistic distribution has a much longer right tail than the standard normal.
Since the asymptotic distribution of the test statistic is not readily found in
theory, we rely on simulation via Algorithm~\ref{alg:sn_q} to estimate the
quantiles.
