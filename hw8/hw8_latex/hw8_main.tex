...
\begin{prop}
  ...
  where $P'$ is the projection matrix onto the null space of $\Xm^\T$,
  $\mathscr{N}(\Xm^\T)$.
\end{prop}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= P'\Xm\Bv + P'\ve.
  \end{align*}
  The projection of $\Xm$ onto the null space of its own transpose $P'\Xm\Bv$ is
  always zero,
...
\begin{proof}
  ...
  \begin{align*}
    \Bvh_\mathrm{LSE} &\coloneqq \argmin_{\Bv \in \R^p} \ve^\T \ve \\
                      &= \argmin_{\Bv \in \R^p} (\Yv - \Xm^\T\Bvh)^\T (\Yv - \Xm^\T\Bvh).
  \end{align*}
  The maximum likelihood estimator is found by
  \begin{align*}
    \Bvh_\mathrm{MLE} &= \argmax_\Bv \prod_{i=1}^n f_x\mleft(Y_i - X_i^\T\Bv\mright) \\
    &\iff \argmax_\Bv \prod_{i=1}^n \frac{1}{\sqrt{2\pi\sigma^2}} \exp \mleft[\frac{-(Y_i - X_i^\T\Bv)^2}{2\sigma^2}\mright] \\
    &\iff \argmax_\Bv \frac{1}{(2\pi\sigma^2)^\frac{n}{2}} 
      \exp \mleft[\frac{-(\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv)}{2\sigma^2}\mright] \\
    &\iff \argmax_\Bv \log \frac{1}{(2\pi\sigma^2)^\frac{n}{2}} 
      \exp \mleft[\frac{-(\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv)}{2\sigma^2}\mright] \\
    &\iff \argmax_\Bv -\frac{1}{2}\mleft[ n\log 2\pi\sigma^2 
          + \frac{1}{\sigma^2} (\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv) \mright] \\
    &\iff \argmin_\Bv \frac{1}{\sigma^2} (\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv) \\
    &\iff \argmin_\Bv (\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv). \\
    \therefore \Bvh_\mathrm{MLE} = \Bvh_\mathrm{LSE} &= \argmin_\Bv (\Yv - \Xm^\T\Bv)^\T(\Yv - \Xm^\T\Bv). \qedhere
  \end{align*}
\end{proof}
